...
export default function PostcardCard({ item, showBack = false }) {
  ...
  const frontBg = data.image
    ? undefined
    : PRESETS[data.preset] || PRESETS.coast
  ...
  return (
    <div
      ...
      style={{
        ...
        height: 180,
        background: '#F1E7C7',
        boxShadow: '3px 4px 12px rgba(74,51,35,0.28), inset 0 0 0 2px rgba(74,51,35,0.12)',
      }}
    >
      ...
      {/* vintage border */}
      <div className="absolute inset-1.5 border border-[#A08560]/40" />

      {!showBack ? (
        <div className="relative h-full w-full overflow-hidden p-3">
          <div
            className="h-full w-full overflow-hidden"
            style={{ background: frontBg || '#2A221C' }}
          >
            {data.image && (
              <img
                src={data.image}
          ...
          {/* stamp */}
          <div
            className="absolute right-4 top-4 flex h-12 w-10 flex-col items-center justify-center"
            style={{
              border: '2px dashed rgba(200,50,46,0.55)',
          ...
          {data.location && (
            <p className="font-hand absolute bottom-4 left-5 text-[1.15rem] text-[#1E3A5F] drop-shadow-sm">
              {data.location}
            </p>
          )}
        </div>
      ) : (
        <div className="relative flex h-full gap-3 p-4 pt-5">
          <div className="flex-1 border-r border-[#A08560]/30 pr-3">
          ...
          <div className="w-[42%]">
            <p className="font-type text-[0.55rem] tracking-wide text-[#6B4A2E]">TO</p>
            ...
            <p className="font-type mt-3 text-[0.55rem] tracking-wide text-[#6B4A2E]">PLACE</p>
            <p className="font-hand text-[1rem] text-[#1E3A5F]">{data.location || '—'}</p>
          </div>
        </div>
      )}
    </div>
